...
class SelfAttention(nn.Module):
    def __init__(self, hidden_dim: int, num_heads: int, dropout: float = 0.1):
        ...
        多头自注意力模块
        Args:
            hidden_dim: 输入特征维度（例如 768）
            num_heads: 注意力头数（必须能整除 hidden_dim）
            dropout: Dropout 概率
        """
    ...
    def forward(self, x: torch.Tensor, attention_mask: torch.Tensor = None):
        ...
        Args:
            ...
            attention_mask: 注意力掩码，形状 (batch_size, seq_len)，1 表示有效，0 表示填充
        Returns:
            output: 注意力输出，形状 (batch_size, seq_len, hidden_dim)
        """
