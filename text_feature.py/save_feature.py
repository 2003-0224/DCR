...
def process_split(model, tokenizer, data_path, split_mode, device, batch_size, max_seq_length, output_dir):
    ...
    if split_mode == "train":
        save_path = os.path.join(output_dir, "train_text_features.npz")
    elif split_mode == "test":
        save_path = os.path.join(output_dir, "test_text_features.npz")
    np.savez(
        save_path,
...
def main():
    ...
    if os.path.exists(stage1_model_path):
        ...
        print(f"Loaded fine-tuned RoBERTa weights from {stage1_model_path}")
    else:
