...
class MultimodalFusionModel(nn.Module):
    def __init__(self, text_dim: int, audio_dim: int, video_dim: int,
        ...
        if self.feature_type == 'pooled_features':
            ...
            # 为不使用原始音频的情况添加卷积层
            if 'A' in modalities and not use_raw_audio:
            ...
            # 简化的池化层
            self.text_pool = nn.AdaptiveAvgPool1d(1) if 'T' in modalities else None
            self.audio_pool = nn.AdaptiveAvgPool1d(1) if 'A' in modalities else None
            self.video_pool = nn.AdaptiveAvgPool1d(1) if 'V' in modalities else None
            ...
            if self.use_cross_modal:
                print("Using lightweight cross-modal attention")
                # 只保留文本-音频交互,减少计算量
                self.crossmodal_ta = CrossModalTransformerEncoder(embed_dim=hidden_dim, num_heads=4, layers=1,
                                                                  attn_dropout=0.1, gelu_dropout=0.1, res_dropout=0.1,
                                                                  embed_dropout=0.1, ) if 'T' in modalities and 'A' in modalities else None
            else:
                print("Cross-modal interaction disabled for simplicity")
            # 简化的注意力机制
            self.attention = AdditiveAttention(hidden_dim, hidden_dim//2)  # 减少注意力维度
            self.dropout = nn.Dropout(0.2)  # 增加dropout防止过拟合
            self.classifier = nn.Linear(hidden_dim, num_classes)
    ...
    def forward(self, inputs: Dict[str, torch.Tensor], labels: torch.Tensor = None):
        if self.feature_type == 'pooled_features':
            ...
            if 'T' in self.modalities and self.use_raw_text:
                ...
                # 简化的序列长度统一：使用平均池化代替转置卷积
                if self.text_pool is not None:
                    text_feat = text_feat.transpose(1, 2)  # (batch_size, hidden_dim, seq_len)
                    text_feat = self.text_pool(text_feat)  # (batch_size, hidden_dim, 1)
                    text_feat = text_feat.transpose(1, 2)  # (batch_size, 1, hidden_dim)
            ...
            if 'A' in self.modalities and self.audio_proj is not None and self.use_raw_audio:
                ...
                if self.audio_pool is not None:
                    audio_feat = audio_feat.transpose(1, 2)  # (batch_size, hidden_dim, seq_len)
                    audio_feat = self.audio_pool(audio_feat)  # (batch_size, hidden_dim, 1)
                    audio_feat = audio_feat.transpose(1, 2)  # (batch_size, 1, hidden_dim)
            ...
            if 'V' in self.modalities and self.video_proj is not None:
                ...
                # 简化的序列长度统一：使用平均池化
                if self.video_pool is not None:
                    video_feat = video_feat.transpose(1, 2)  # (batch_size, hidden_dim, seq_len)
                    video_feat = self.video_pool(video_feat)  # (batch_size, hidden_dim, 1)
                    video_feat = video_feat.transpose(1, 2)  # (batch_size, 1, hidden_dim)
            ...
            if self.use_cam_loss:
                # 如果必须使用CAM损失,使用简化的版本
                print("WARNING: CAM loss is enabled but may hurt performance")
                cam_loss = 0.0  # 返回0损失,避免复杂的KL散度计算
            ...
            if self.use_cross_modal:
                ...
                if 'V' in self.modalities:
                    ...
                    final_feat = torch.cat([ta_cross_feat, video_feat], dim=0)  # 直接拼接
                else:
                ...
                pooled_feats = []
                if 'T' in self.modalities:
                    pooled_feats.append(text_feat.mean(dim=1, keepdim=True))  # (batch_size, 1, hidden_dim)
                if 'A' in self.modalities:
                    pooled_feats.append(audio_feat.mean(dim=1, keepdim=True))  # (batch_size, 1, hidden_dim)
                if 'V' in self.modalities:
                    pooled_feats.append(video_feat.mean(dim=1, keepdim=True))  # (batch_size, 1, hidden_dim)
                combined_feat = torch.cat(pooled_feats, dim=1)  # (batch_size, N_mod, hidden_dim)
                final_feat = combined_feat.transpose(0, 1)  # (N_mod, batch_size, hidden_dim)
                final_feat = final_feat.transpose(0, 1)  # (batch_size, seq_len, hidden_dim)
                multimodal_out, _ = self.attention(final_feat)  # (batch_size, hidden_dim)
                multimodal_out = self.dropout(multimodal_out)
                logits = self.classifier(multimodal_out)
