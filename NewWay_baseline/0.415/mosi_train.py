...
# --- 1. 导入清晰化：直接导入 MOSI 适配的 Dataset 类 ---
from feature_mosi_dataset import MOSIDataset  # 预提取特征数据集
from T_raw_MOSIDataset import T_raw_MOSIDataset  # 原始文本（和V/A特征）数据集
from TA_raw_MOSIDataset import TA_raw_MOSIDataset  # 原始文本+原始音频（和V特征）数据集
# ---------------------------------------------------
...
def main():
    # Parameters - 优化版
    seeds = [42]
    # num_labels 已经正确设置为 7
    num_labels = 7
    num_epochs = 64
    batch_size = 32
    lr = 1e-4  # 增大学习率（原1e-6太小）
    eta_min = 1e-5  # 调整最小学习率
    patience = 10
    ...
    use_raw_text = True
    use_cam_loss = False  # 禁用复杂的CAM损失
    use_raw_audio = False
    whisper_use_adapters = True
    cam_type = "T_to_CAM"  # 简化CAM类型
    ...
    # --- 2. 数据路径/特征文件名更新，匹配 MOSI 任务 ---
    data_paths = {
        'train': {
            'text': '/data/home/chenqian/models/features/train_valid_text_features.npz',
            'text_raw_path': '/data/home/chenqian/CMU_MOSI/CMU-MOSI/label.csv',
            'audio': '/data/home/chenqian/models/features/train_audio_features.npz',
            'video': '/data/home/chenqian/models/features/train_video_features.npz',
            'audio_csv_path': '/data/home/chenqian/CMU_MOSI/CMU-MOSI/all_train_data.csv',
            'audio_data_path': '/data/home/chenqian/CMU_MOSI/CMU-MOSI/all_train_data.csv'
        },
        'test': {
            'text': '/data/home/chenqian/models/features/test_text_features.npz',
            'text_raw_path': '/data/home/chenqian/CMU_MOSI/CMU-MOSI/label.csv',
            'audio': '/data/home/chenqian/models/features/test_audio_features.npz',
            'video': '/data/home/chenqian/models/features/test_video_features.npz',
            'audio_csv_path': '/data/home/chenqian/CMU_MOSI/CMU-MOSI/all_test_data.csv',
            'audio_data_path': '/data/home/chenqian/CMU_MOSI/CMU-MOSI/all_test_data.csv'
        }
    }
    # -------------------------------------------------------------
    ...
    def build_dataset(split: str):
        ...
        if use_raw_text and use_raw_audio:
            # 使用 TA_raw_MOSIDataset
            return TA_raw_MOSIDataset(
                cfg['text'],
                cfg['audio'],
                cfg['video'],
                modalities,
    ...
    for seed in seeds:
        ...
        # Optimizer, scheduler, and EMA - 优化版
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-4)  # 使用AdamW
        scheduler = CosineAnnealingLR(optimizer, T_max=16, eta_min=eta_min)  # 增大T_max
        ...
        for epoch in range(num_epochs):
            train_loss = train(model, train_loader, optimizer, scheduler, ema, device, use_cam_loss=use_cam_loss,
                               use_raw_audio=use_raw_audio, cam_type=cam_type)
            # 评估函数 evaluate 应该已经适配了 MOSI 7-class 的评估指标
            ...
            if test_weighted_f1 > global_best_f1:
                ...
                ema.restore()
                print(
                    f"New global best found: Test Accuracy={global_best_acc:.4f},  Weighted-F1={global_best_f1:.4f},  Seed={seed}, Epoch={epoch + 1}")
        ...
        if torch.cuda.is_available():
            ...
            print(
                f"GPU memory cleared. Current allocated memory: {torch.cuda.memory_allocated(device) / 1024 ** 2:.2f} MB")
            print(f"GPU memory cached: {torch.cuda.memory_reserved(device) / 1024 ** 2:.2f} MB")

    # Save global best model
    if global_best_acc > 0:
        save_path = f"/data/home/chenqian/models/multimodal_fusion_best_acc_{global_best_acc:.4f}_seed_{global_best_seed}.pth"
        torch.save(global_best_model_state, save_path)
        print(f"\nSaved global best model parameters to: {save_path}")
        print(
            f"Global Best Test Accuracy: {global_best_acc:.4f}, Weighted-F1: {global_best_f1:.4f}, Seed: {global_best_seed}")

        # Collect logits and predictions for later use
