...
def evaluate_agent(agent, experts, data_loader, device, num_labels, num_actions, agent_feature_store, default_features,
                   multimodal_store=None):
...
def main():
    ...
    expert_model_paths = {
        'T': '/data/yuyangchen/checkpoints/unimodal_experts/unimodal_t_best_f1_0.6750_seed_42.pth',
        'A': '/data/yuyangchen/checkpoints/unimodal_experts/unimodal_a_best_f1_0.4124_seed_42.pth',
        # 这里可以换成过拟合的模型，再次进行预测
    ...
    agent_feature_paths = {
        'train': {
            'T': '/data/yuyangchen/data/MELD/text_features/train_text_features_general.npz',
            'A': '/data/yuyangchen/data/MELD/audio_features/train_audio_features_general.npz',
            'V': '/data/yuyangchen/data/MELD/face_features/train_face_features_general_clip.npz',
        },
        'test': {
            'T': '/data/yuyangchen/data/MELD/text_features/test_text_features_general.npz',
            'A': '/data/yuyangchen/data/MELD/audio_features/test_audio_features_general.npz',
            'V': '/data/yuyangchen/data/MELD/face_features/test_face_features_general_clip.npz',
        }
    }
    ...
    multimodal_train_npz = '/data/yuyangchen/checkpoints/multimodal_fusion_best_acc_0.6893_seed_42_train_predictions.npz'
    multimodal_test_npz = '/data/yuyangchen/checkpoints/multimodal_fusion_best_acc_0.6893_seed_42_test_predictions.npz'
    ...
    # 修改entropy_coef
    # base_save_dir = "/data2/home/chenqian/checkpoints/entropyCoef"
    # 修改value_coef
    # base_save_dir = "/data2/home/chenqian/checkpoints/valueCoef"
    # 类别奖励加权 (Class-Weighted Reward)
    base_save_dir = "/data2/home/chenqian/checkpoints/class_weighted_reward"
    ...
    for seed in seeds:
        ...
        set_seed(seed)
        # 获取训练集标签
        train_labels = train_dataset.labels.flatten().astype(int)
        # 统计每个类别样本数
        class_counts = np.bincount(train_labels)
        # 计算逆频率权重 (用于 Reward 加权: 逆频率)
        class_weights_np = 1.0 / np.sqrt(np.maximum(class_counts, 1))  # 防止除零
        # 将 NumPy 数组转换为 PyTorch Tensor 并移至设备
        class_weights_tensor = torch.from_numpy(class_weights_np).float().to(device)
        # 恢复标准 DataLoader (不重采样, 使用 shuffle=True)
        ...
        for epoch in range(num_epochs):
            ...
            for batch in tqdm(train_loader, desc=f"Seed {seed} Epoch {epoch + 1}/{num_epochs}"):
                ...
                with torch.no_grad():
                    ...
                    # --- [修改开始]：类别奖励加权 ---
                    # 获取当前批次样本对应的类别奖励权重
                    reward_weights_for_batch = class_weights_tensor[labels.long()]
                    # 应用类别加权奖励: 正确预测时, 奖励 = 置信度 * 类别权重; 错误预测时, 奖励 = -置信度 (不变)
                    rewards = torch.where(correct_mask, chosen_probs * reward_weights_for_batch, -chosen_probs)
                    # --- [修改结束] ---
