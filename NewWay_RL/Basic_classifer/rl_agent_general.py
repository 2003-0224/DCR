...
class GeneralMemoryEncoder(nn.Module):
    """Encode modality-specific general features into the agent embedding space."""
    ...
    def __init__(self, input_dim, embed_dim, pooling='mean', conv_kernel_size=3):
        ...
        if pooling == 'conv1d':
            self.encoder = nn.Sequential(
                nn.Conv1d(input_dim, embed_dim, kernel_size=conv_kernel_size, padding=conv_kernel_size // 2, bias=False),
    ...
    def forward(self, memory):
        ...
        memory = memory.float()
        memory = memory.transpose(1, 2)  # (B, input_dim, L)
        ...
        encoded = encoded.transpose(1, 2)  # (B, L, embed_dim)
        encoded = self.norm(encoded)
        return encoded


class RLAgent(nn.Module):
    """多模态特征选择智能体，使用专家特征作为查询检索通用特征。"""
    ...
    def __init__(
        ...
        if not self.modalities:
            raise ValueError("modality_dims 不能为空。")
        ...
        self.missing_embeddings = nn.ParameterDict({
            m: nn.Parameter(torch.zeros(embed_dim))
        ...
        for modality in self.modalities:
            ...
            self.modality_attn[modality] = nn.MultiheadAttention(embed_dim, num_heads, batch_first=True, dropout=dropout)
    ...
    def forward(self, features, return_repr=False):
        if not isinstance(features, dict):
            raise TypeError("features 必须是包含各模态输入的字典。")
        ...
        for modality in self.modalities:
            ...
            if query is None:
                if not isinstance(memory, torch.Tensor):
                    raise KeyError(f"Modality '{modality}' requires at least tensor inputs to infer query")
                if memory.dim() == 3:
            ...
            if self.include_memory_tokens:
                mem_tokens = memory_tokens + self.mod_embeddings[self.modality_to_idx[modality]].view(1, 1, -1)
                if mask is not None:
