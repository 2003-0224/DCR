...
def process_video_features(
    ...
    aligned_pooled_features = np.array(aligned_pooled_features,
                                       dtype=np.float32) if aligned_pooled_features else np.array([])
    aligned_sequence_features = np.array(aligned_sequence_features,
                                         dtype=np.float32) if aligned_sequence_features else np.array([])
...
def main():
    # Define paths
    base_dir = "/data/yuyangchen/data/MELD"
