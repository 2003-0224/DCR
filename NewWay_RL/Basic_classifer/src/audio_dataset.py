...
from transformers import Wav2Vec2FeatureExtractor, Wav2Vec2Processor, AutoModelForSpeechSeq2Seq, \
    AutoProcessor  # Data2VecAudio使用相同的Processor
...
class Dataset_meld_audio(Dataset):
    def __init__(self, csv_path, audio_directory, max_length=96000, cache_dir=None, model_type=None):
        ...
        for i in range(len(df)):
            ...
            if os.path.exists(file_path):
                ...
                self.sample_names.append(f"dia{dialogue_id}_utt{utterance_id}")  # 保存样本名，例如 diaX_uttY
            else:
                print(f"Warning: Audio file {file_path} not found.")
                self.audio_file_paths.append(None)
                self.sample_names.append(f"dia{dialogue_id}_utt{utterance_id}")  # 即使文件缺失，也保留样本名

        self.sampling_rate = 16000
        self.max_length = max_length
        # self.processor = Wav2Vec2FeatureExtractor(feature_size=1, sampling_rate=16000, padding_value=0.0,
        # do_normalize=True, return_attention_mask=True) self.processor = Wav2Vec2FeatureExtractor.from_pretrained(
        ...
        # # 检查缓存是否已生成
        # self.cached_data = self._load_cached_data() if self._is_cache_complete() else self._load_or_cache_data()
        self.cached_data = self._load_data()
        for i in range(len(self.cached_data)):
            if self.cached_data[i]['input_values'].shape[0] != 128:
                # print(f"Warning: Data at index {i} has shape {self.cached_data[i]['input_values'].shape},
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            "sample_name": self.sample_names[index]  # 添加样本名
        }
...
def data_loader_meld_audio(batch_size, max_seq_length, model_type, num_workers=4,
                           cache_dir="/data/yuyangchen/data/MELD/cache", ):
    base_path = '/data/yuyangchen/data/MELD'
    audio_base_path = '/data/yuyangchen/data/MELD'
