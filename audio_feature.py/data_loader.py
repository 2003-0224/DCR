...
#         else:
#             raise KeyError(
#                 "CSV file must contain an emotional score column named 'label', 'score', or 'MOSI情感分数'. Please check "
...
#         else:
#             raise ValueError(
#                 "CSV must contain either 'audio_path' or both 'video_id' and 'clip_id' columns, and 'audio_directory' "
...
#             else:
#                 print(
#                     f"Warning: local_model_path not found or provided. Loading Whisper Processor from HuggingFace Hub "
#                     f"(openai/whisper-large-v3)")
...
#                     num_mels = self.processor.feature_extractor.feature_size
...
#
# # --- MOSI 数据加载器函数 (保持不变) ---
# def data_loader_mosi_audio(train_csv_path,valid_csv_path, test_csv_path, audio_directory=None, batch_size=32, max_seq_length=96000,
#                            model_type=None, num_workers=4,
#                            cache_dir="/data/home/chenqian/cache", local_model_path="/data/home/chenqian"
#                                                                                    "/whisper_large_v3/"):
#     # 训练集
...
#     valid_loader = DataLoader(valid_data, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)
...
class MOSIAudioDataset(Dataset):
    def __init__(self, csv_path, audio_directory=None, max_length=96000, cache_dir=None, model_type=None,
        ...
        if cache_dir and not os.path.exists(cache_dir):
            os.makedirs(cache_dir)
        self.targets_vscore = self.df[label_col].astype(float).tolist()
        self.sampling_rate = 16000
        self.max_length = max_length
        if 'audio_path' in self.df.columns:
            # 模式 A: 使用 'audio_path' 列中的绝对路径
    ...
    def __getitem__(self, index):
        label = self.targets_vscore[index]
        data = self.cached_data[index]

        return {
            "input_values": data["input_values"],
            "attention_mask": data["attention_mask"],
            "label": torch.tensor(label, dtype=torch.float32).unsqueeze(0),  # 确保形状为 [1]
            "sample_name": self.sample_names[index]
        }
...
def data_loader_mosi_audio(train_csv_path, valid_csv_path, test_csv_path, audio_directory=None, batch_size=32,
                           max_seq_length=96000,
                           model_type=None, num_workers=4,
                           cache_dir="/data/home/chenqian/cache",
