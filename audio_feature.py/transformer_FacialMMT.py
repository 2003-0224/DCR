...
class SelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        attention_scores = attention_scores / math.sqrt(
            self.attention_head_size)  #(batch_size, 12, embed_dim, embed_dim)
        '''
...
class MELDTransEncoder(nn.Module):
    def __init__(self, config, layer_num, get_max_lens, hidden_size):
        ...
        self.position_embeddings = nn.Embedding(get_max_lens,
                                                hidden_size)  #初始化定义position_embedding的embedding层，其实底层就是一个fc
