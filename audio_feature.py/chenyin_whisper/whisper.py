...
def create_custom_processor(model_name="openai/whisper-small", chunk_length=3):
    """创建自定义的processor，适应较短的输入长度"""
    feature_extractor = WhisperFeatureExtractor.from_pretrained(
        model_name,
        chunk_length=chunk_length,  # 多少秒
    )
...
class CustomWhisperModel(BaseModel):
    def __init__(self, num_classes=0, model_name="openai/whisper-small.en", chunk_length=5, adapter_dim=192,
                 adapter_scale=0.1):
        super(CustomWhisperModel, self).__init__()

        # 创建自定义processor
        # self.processor = create_custom_processor(model_name=model_name, chunk_length=chunk_length)
        ...
        max_positions = int(chunk_length * 100 / 2)  # 略大于你的最大长度30
...
if __name__ == "__main__":
    ...
    # 处理音频数据
    audio_signal = np.random.randn(57000)  # 示例音频数据
